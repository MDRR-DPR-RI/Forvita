    ...
    <div id="sidebarMenu" class="sidebar-body">
        <div class="nav-group show">
            @if (session()->has('cluster_id'))
                @can('admin')
                    <!-- using gate in AppServiceProvider to show this menu only for admin-->
                    <a href="#" class="nav-label">Admin</a>
                    <ul class="nav nav-sidebar">
                        <li class="nav-item">
                            <a href="/database"
                                class="nav-link @isset($databases) @empty($schedulers) active @endempty @endisset"><i
                                    class="bi bi-database-fill-gear"></i> <span>Databases</span></a>
                        </li>
                        <li class="nav-item">
                            <a href="/scheduler" class="nav-link @isset($schedulers)  active  @endisset"><i
                                    class="bi bi-gear-wide-connected"></i> <span>Queries</span></a>
                        </li>
                        <li class="nav-item">
                            <a href="/csv" class="nav-link @isset($csvFiles) active @endisset"><i
                                    class="ri-file-excel-2-line fs-18 lh-1"></i> <span>CSV List</span></a>
                        </li>
                        <li class="nav-item">
                            <a href="/restapi" class="nav-link @isset($apiList) active @endisset"><i
                                    class="bi bi-link-45deg"></i> <span>API List</span></a>
                        </li>
                        <li class="nav-item">
                            <a href="/user-management"
                                class="nav-link @isset($initialUsers) active @endisset"><i
                                    class="bi bi-person-fill-gear"></i> <span>Manajemen Pengguna</span></a>
                        </li>
                        <li class="nav-item">
                            <a href="/data-table" class="nav-link @isset($datatables) active @endisset"><i
                                    class="ri-file-excel-2-line fs-18 lh-1"></i> <span>Tabel Data</span></a>
                        </li>
                        <li class="nav-item">
                            <a href="/excel" class="nav-link @isset($excel) active @endisset"><i
                                    class="ri-file-excel-2-line fs-18 lh-1"></i> <span>Import Excel</span></a>
                        </li>
                    </ul>
                @endcan
                ...
                <ul class="nav nav-sidebar">
                    ...
                    @foreach ($dashboards as $index_dashboard)
                        <li class="nav-item">
                            @if (isset($dashboard->name))
                                <a href="/dashboard/{{ $index_dashboard->id }}"
                                    class="nav-link  {{ $dashboard->id == $index_dashboard->id ? 'active' : '' }}"><i
                                        class="{{ $index_dashboard->icon_name }}"></i>
                                    <span>{{ $index_dashboard->name }}</span></a>
                            @else
                                <a href="/dashboard/{{ $index_dashboard->id }}" class="nav-link"><i
                                        class="{{ $index_dashboard->icon_name }}"></i>
                                    <span>{{ $index_dashboard->name }}</span></a>
                            @endif
                        </li>
                    ...
                    @can('user')
                        <li class="nav-item">
                            <a href="#newDashboard" data-bs-toggle="modal" class="nav-link "><span
                                    class="btn btn-secondary btn-sm"></i>Tambah Dashboard</span></a>
                        </li>
                    @endcan
                </ul>
            @endif
            @can('admin')
                ...
                <ul class="nav nav-sidebar">
                    <li class="nav-item">
                        <a href="#share-list" class="nav-link" data-bs-toggle="modal"><i class="ri-global-line"></i>
                            <span>Dashboard Publik</span></a>
                    </li>
                </ul>
            @endcan
        </div><!-- nav-group -->
            ...
            <div class="modal-content">
                ...
                <form action="/dashboard" method="post">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <input type="text" class="form-control" name="dashboard_name" placeholder="Nama dashboard"
                            autofocus required>
                    </div>
                    <div class="modal-body text-center">
                        <label>Masukan Deskripsi Dashboard:</label>
                        <textarea class="form-control" name="dashboard_description" rows="3" placeholder="Deskripsi dashboard..."
                            required></textarea>
                    </div>
                    <div class="modal-body text-center">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" name="icon" class="iconInputs form-control iconpicker"
                                ...
                                <tbody>
                                    @foreach ($shares as $key => $share)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="https://172.18.25.16/public/dashboard/{{ $share->link }}"
                                                    target="_blank">https://172.18.25.16/public/dashboard/{{ $share->link }}</a>
                                            </td>
                                            <td>{{ $share->expired }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center align-items-center">
                                                    <a href="/share/{{ $share->id }}" class="btn btn-primary btn-icon"
