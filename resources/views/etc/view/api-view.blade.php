...
@section('page_content')
    ...
    <script>
        ...
        fetch('/restapi/view/target?id={{ $_GET['id'] }}')
            .then(response => response.text())
            .then(apiData => {
                // Proses data menggunakan Papaparse
                Papa.parse(apiData, {
                    ...
                    complete: function(results) {
                        ...
                        var head = Object.keys(apiObject[0]);

                        var row = Object.values(apiObject).map(Object.values);
                        var localData = row.map(function(dataRow) {
                            return Object.assign(...head.map((key, index) => ({
                                [key]: dataRow[index]
                            })));
                        });
                        var headColumn = []
